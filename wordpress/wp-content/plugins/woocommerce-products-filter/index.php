...
//
    ...
    public function woof_products($atts, $is_prediction = false) {
        ...
        if (isset($_REQUEST['perpage'])) {
            ...
            {
                $args['posts_per_page'] = intval($_REQUEST['perpage']);
            }
        }
        ...
        public function woof_draw_products() {
            ...
            if (isset($_REQUEST['woof_shortcode'])) {//if search form on the page exists
                $text = "";
                $shortcode_str = "";
                $woof_shortcode_str = wp_kses_post(wp_unslash($_REQUEST['woof_shortcode']));
                if (empty($_REQUEST['woof_additional_taxonomies_string'])) {
                    $text = "[" . stripslashes($woof_shortcode_str) . "]";
                } else {
                    $taxonomies_str = sanitize_text_field($_REQUEST['woof_additional_taxonomies_string']);
                ...
                if (!empty($shortcode_str)) {
                    $_REQUEST['woof_shortcode_txt'] = $woof_shortcode_str;
                }
                //***
        ...
        public function woof_modify_query_args($query_args) {
            ...
            if (isset($_REQUEST[$this->get_swoof_search_slug()])) {
                if (isset($_REQUEST['woof_wp_query_args'])) {
                    ...
                    $query_args['meta_query'] = $WP_Meta_Query->sanitize_query( $_REQUEST['woof_wp_query_args']['meta_query'] );
                                       
                    $WP_Tax_Query = new WP_Tax_Query();
                    $query_args['tax_query'] =  $WP_Tax_Query->sanitize_query( $_REQUEST['woof_wp_query_args']['tax_query'] );
                    
                    $query_args['paged'] = intval($_REQUEST['woof_wp_query_args']['paged']);
                }
            }
