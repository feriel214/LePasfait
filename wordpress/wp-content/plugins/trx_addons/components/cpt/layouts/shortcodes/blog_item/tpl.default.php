...
?>"<?php
	if (!wp_is_mobile() && (!trx_addons_is_off($args['animation_in']) || !trx_addons_is_off($args['animation_out']))) {
		...
		if (!trx_addons_is_off($args['animation_in'])) echo ' data-animation-in="'.esc_attr($args['animation_in']).'"';
		if (!trx_addons_is_off($args['animation_out'])) echo ' data-animation-out="'.esc_attr($args['animation_out']).'"';
	}
...
?>>
	...
	if ($args['type'] == 'title') {
		...
		if ( function_exists('trx_addons_elm_is_preview') && trx_addons_elm_is_preview() && !trx_addons_sc_stack_check('trx_sc_blogger') ) {
		...
		if ( $is_preview ) {
			...
			$post_meta = '<div class="post_meta">' . implode(' / ', $args['meta_parts']) . '</div>';
		} else {
