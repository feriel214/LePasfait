...
if (!function_exists('trx_addons_add_scroll_to_top')) {
	...
	function trx_addons_add_scroll_to_top() {
		if (trx_addons_is_on(trx_addons_get_option('scroll_to_top'))) {
			if ( function_exists('vapester_exists_wp_live_chat_support') && vapester_exists_wp_live_chat_support() ){
				?><a href="#" class="trx_addons_scroll_to_top trx_addons_icon-up wplc-chat-button" title="<?php esc_attr_e('Scroll to top', 'trx_addons'); ?>"></a><?php
			} else {
				...
				if (!is_singular() || have_comments() || comments_open()) {
					...
					$output .= ' ' 
								. '<a href="'.esc_url(get_comments_link()).'" class="post_counters_item post_counters_comments trx_addons_icon-comment">'
									...
									. '<span class="post_counters_label">' . (1==$post_comments ? esc_html__('Comment', 'trx_addons') : esc_html__('Comments', 'trx_addons')) . '</span>'								. '</a>'
								. ' ';
				}
			} else if ($comp == 'views') {
