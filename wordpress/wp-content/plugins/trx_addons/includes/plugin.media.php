...
if (!function_exists('trx_addons_get_slider_layout')) {
	function trx_addons_get_slider_layout($args=array(), $images=array()) {
		$args = apply_filters('trx_addons_filter_slider_args', array_merge(array(
			...
			'slides_space' => 50,			// Space between slides
...
//											: '')
									...
									. '"'
								. '>';
				if ( empty($image['title']) && empty($image['subtitle']) && !empty($image['image']) && empty($image['video_url']) && empty($image['video_embed']) && !empty($image['image_extra']) ) 
					$output .= '<div class="slider_only_image">';
								$output .= ($args['slides_type'] == 'bg' || empty($image['url']) 
									? '' 
			...
			$button = '';
			// Prepare button
			if ( isset($args['link']) && $args['link'] && isset($args['link_text']) && $args['link_text'] ) {
				$btn_style = ( isset($args['link_style']) && $args['link_style'] ) ? $args['link_style'] : 'default';
				ob_start(); ?>
					<div class="slider_button"><a href="<?php echo $args['link'] ?>" class="sc_action_item_link sc_button sc_button_size_medium sc_button_<?php echo $btn_style ?> sc_button_hover_slide_left"><?php echo $args['link_text'] ?></a></div>
